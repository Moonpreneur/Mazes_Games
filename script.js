...
function DrawMaze(Maze, ctx, cellsize, endSprite = null) {
  ...
  function drawCell(xCord, yCord, cell) {
    ...
    const borderColors = ['#FF5733','#33FF57'];
    let colorIndex = 0;

    // Function to draw border lines only
    function animateBorderLines() {
        ctx.strokeStyle = borderColors[colorIndex]; // Change color for borders
        ctx.lineWidth = cellSize / 20;  // Set border width
        colorIndex = (colorIndex + 1) % borderColors.length;

        // Draw borders but not inside the paths
        // Draw north border
        if (cell.n === false) {
            ctx.beginPath();
            ctx.moveTo(x, y);
    ...
    // Start border animation with interval
    animateBorderLines();
    setInterval(animateBorderLines, 2000);  // Update color every 2 seconds
}
...
function Player(maze, c, _cellsize, onComplete, sprite = null) {
  ...
  var moveSound = new Audio('tap.mp3');
  ...
  function check(e) {
    ...
    // Update the moves display in the top-left corner
...
var backgroundMusic = new Audio('me.mp3');
...
window.onload = function () {
  ...
  sprite = new Image();
  sprite.src = "https://raw.githubusercontent.com/Moonpreneur/Maze_Game/refs/heads/main/images.jfif";
  ...
  finishSprite.src = "https://raw.githubusercontent.com/Moonpreneur/Maze_Game/refs/heads/main/bone.webp";
...
function startGame(level) {
  ...
  document.getElementById('movesDisplay').style.display = 'block';
...
function displayVictoryMess(moves) {
   document.getElementById('gameOverMessage').innerHTML = "You completed the maze in " + moves + " moves!";
   ...
   setTimeout(function () {
    ...
    document.getElementById('movesDisplay').style.display = 'none';
 }, 4000); 
}
...
speaker.addEventListener("click", function () {
  if (backgroundMusicPlaying) {
    backgroundMusic.pause();
    backgroundMusic.currentTime = 0;
    speakerIcon.src = "speaker.png";
    speakerIcon.classList.remove("green");
    speakerIcon.classList.add("red");
  } else {
    backgroundMusic.play();
    speakerIcon.src = "speaker.png"; 
...
function applyZoom() {
  ...
  mazeContainer.style.transform = `scale(${zoomLevel / 100})`;
  mazeContainer.style.transformOrigin = "top left";

if (zoomLevel > 107) {
    mazeContainer.style.overflow = "auto";
  } else {
    mazeContainer.style.overflow = "hidden";
    mazeContainer.style.width = "100vw";
    mazeContainer.style.height = "100vh";
  }
  zoomIndicator.textContent = `${zoomLevel}%`;
  zoomIndicator.style.backgroundColor = zoomLevel === 107 ? "darkgreen" : "red";
}
